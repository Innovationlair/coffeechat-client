...
.service('Helpers', function($q){
	this.getNetworkInfo = function(){
	    ...
	    navigator.wifi.getConnectedWifiInfo(
    		function(success){
    			console.log("success on get wifi " + success);
	    		deferred.resolve(success)
	    	}, function(error){
	    		console.log("error on get wifi info " + error);
	...
	this.getPosition = function(){
		...
		navigator.geolocation.getCurrentPosition(
			function(position){
				console.log("position okay " + position);
				deferred.resolve(position);
			}, function(error){
				console.log("error on get position " + position);
				deferred.reject(new Error(error));
		});
